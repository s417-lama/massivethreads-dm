...
if [[ "$MACHINE_NAME" == ito ]]; then
  CCFLAGS=$CFLAGS CXXFLAGS=$CFLAGS ./configure --with-comm-layer=mpi3 "${logger_options[@]}" --prefix=${INSTALL_DIR}
  ...
  CFLAGS="-Nclang"
  CCFLAGS=$CFLAGS CXXFLAGS=$CFLAGS ./configure MPICC=mpifccpx MPICXX=mpiFCCpx --host=aarch64 --with-comm-layer=mpi3 "${logger_options[@]}" --prefix=${INSTALL_DIR}
  ...
  CCFLAGS=$CFLAGS CXXFLAGS=$CFLAGS ./configure --with-comm-layer=mpi3 --enable-polling "${logger_options[@]}" --prefix=${INSTALL_DIR}
