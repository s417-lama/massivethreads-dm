...
namespace madi {
    ...
    void handle_steal_request(int tag, int pid, void *p, size_t size,
                              aminfo *info);
    void handle_steal_reply(int tag, int pid, void *p, size_t size,
                            aminfo *info);
    ...
    {
        switch (tag) {
        case uth_comm::AM_STEAL_REQ:
            handle_steal_request(tag, pid, data, size, info);
            return true;
        case uth_comm::AM_STEAL_REP:
            handle_steal_reply(tag, pid, data, size, info);
            return true;
        default:
            MADI_NOT_REACHED;
        }
